...
public class CubBoardEvents implements Listener {

    private final Main plugin;

    public CubBoardEvents(Main plugin) {
        this.plugin = plugin;
    }

    @EventHandler
    public void onCubBoardBreak(BlockBreakEvent event) {
        CubBoard cubBoard = plugin.databaseManager.getCupBoard(new PluginLocation(event.getBlock().getLocation()), true);
        if (cubBoard == null) {
            return;
        }
        cubBoard.delete();

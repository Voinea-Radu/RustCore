...
public class OpenGUI extends GUIFunction {
    ...
    public void execute(GUI gui, User user, List<String> args) {
        ...
        switch (menu) {
            case "cub_board":
                new CubBoardGUI(Main.instance, user,((WithArgs) gui).getArg(CubBoard.class)).open(user);
                break;
            case "cub_board_players":
                new CubBoardPlayersGUI(Main.instance, user, ((WithArgs) gui).getArg(CubBoard.class)).open(user);
                break;
            case "crafting_general":
                new CraftingGUI(Main.instance, user, menu).open(user);
                break;
            case "crafting_food":
                new CraftingGUI(Main.instance, user, menu).open(user);
                break;
